...
class LowFreqExtractor(nn.Module):
    def __init__(self, dim, stage2 =True):
        super(LowFreqExtractor, self).__init__()
        self.net1 = C2f(c1=dim, c2=dim, n=3, g=int(dim//2), e=0.5)
        self.net2 = C2f(c1=dim, c2=dim, n=3, g=int(dim//2), e=0.5)
...
class DE_Decoder(nn.Module):
    ...
    def forward(self, inp_img, base_feature, detail_feature, is_2dfusion=False):
        if is_2dfusion:
            embedded_feature = base_feature
        else: 
